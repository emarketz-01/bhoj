<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
	<div class="entry post clearfix<?php if ( get_option('leanbiz_show_postcomments') == 'false' || 0 == get_comments_number() ) echo ' comments_disabled'; ?>">
		...
		<?php if ( get_option('leanbiz_postinfo2') && ( in_array( 'author', get_option('leanbiz_postinfo2') ) || in_array( 'categories', get_option('leanbiz_postinfo2') ) ) ) { ?>
			<p class="meta-info"><?php esc_html_e('Posted','LeanBiz'); ?><?php if (in_array('author', get_option('leanbiz_postinfo2'))) { ?> <?php esc_html_e('by','LeanBiz'); ?> <?php the_author_posts_link(); ?><?php } ?><?php if (in_array('categories', get_option('leanbiz_postinfo2'))) { ?> <?php esc_html_e('in','LeanBiz'); ?> <?php the_category(', ') ?><?php } ?></p>
		<?php } ?>
