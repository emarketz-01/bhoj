...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'leanbiz_featured_num' ),
			'cat'				=> get_catId( et_get_option( 'leanbiz_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', et_get_option( 'leanbiz_homepage_posts', '6' ) );
	
	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'leanbiz_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'leanbiz_featured', 'on' ) && 'false' == et_get_option( 'leanbiz_use_pages', 'false' ) && 'false' == et_get_option( 'leanbiz_duplicate', 'on' ) )
